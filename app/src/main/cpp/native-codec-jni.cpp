...
#include "player/IsmartvPlayerJni.h"
...
//                                                         static_cast<off64_t>(outLen));
    ...
    media_status_t err = AMediaExtractor_setDataSource(ex, d->source);
...
// set the surface
void
Java_com_example_nativecodec_NativeCodec_setSurface(JNIEnv *env, jclass clazz, jobject surface) {
...
Java_cn_ismartv_player_IsmartvPlayer_prepare(JNIEnv *env, jobject instance, jstring source_) {
    const char *source = env->GetStringUTFChars(source_, 0);

    data.source = (char *) malloc(strlen(source) + 1);

    strcpy(data.source, source);
    IsmartvPlayer_init(env);

    ALOGD("Java_cn_ismartv_player_IsmartvPlayer_prepare");
